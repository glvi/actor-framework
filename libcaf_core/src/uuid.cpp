...
void append_hex_backport(std::string& result, const void* vptr, size_t n) {
  if (n == 0) {
    result += "00";
    return;
  }
...
error parse(string_view str, uuid& dest) {
  ...
  switch (parse_impl(ps, dest.bytes())) {
    ...
    case invalid_version:
      return make_error(pec::illegal_argument,
...
std::string to_string(const uuid& x) {
  ...
  std::string result;
  append_hex_backport<lowercase>(result, x.bytes().data() + 0, 4);
  result += '-';
  append_hex_backport<lowercase>(result, x.bytes().data() + 4, 2);
  result += '-';
  append_hex_backport<lowercase>(result, x.bytes().data() + 6, 2);
  result += '-';
  append_hex_backport<lowercase>(result, x.bytes().data() + 8, 2);
  result += '-';
  append_hex_backport<lowercase>(result, x.bytes().data() + 10, 6);
  return result;
}
